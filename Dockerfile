...
#RUN service ssh start && $HADOOP_PREFIX/sbin/start-dfs.sh && $HADOOP_PREFIX/sbin/start-yarn.sh && $HADOOP_PREFIX/bin/hadoop dfsadmin -safemode leave && $HADOOP_PREFIX/bin/hadoop fs -chmod 777 / && $HADOOP_PREFIX/bin/hdfs dfs -mkdir -p /user/user && $HADOOP_PREFIX/bin/hadoop fs -chown -R user:supergroup /user/user
